defmodule Venture.ChatChannelTest do
  use Venture.ChannelCase

  alias Venture.ChatChannel
  ...
  setup context do
    socket = socket("user:#{@uuid}", %{uuid: @uuid})
  ...
  test "doesn't allow duplicate nicks", %{socket: socket} do
    bert = socket("user:b-#{@uuid}", %{uuid: "b-#{@uuid}"})
